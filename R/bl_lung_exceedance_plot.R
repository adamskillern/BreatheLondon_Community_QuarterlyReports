...
BL_LUNG_TRACHEA_TOP <- 0.985

bl_lung_trachea_segments <- function() {
  rail_l <- 0.478
  rail_r <- 0.522
  top <- BL_LUNG_TRACHEA_TOP
  split <- 0.855
...
bl_lung_plot_fig_height <- function(n_years, years_per_row = 3L, row_height = 4.4) {
  row_height * max(1L, ceiling(as.integer(n_years) / years_per_row))
}

bl_lung_year_layers <- function(
    year_row,
    outlines,
    yranges,
    no2_color,
    pm25_color) {
  yr <- as.character(year_row$year)
  sides <- list(
    list(side = "left", pct = year_row$no2_pct, col = no2_color),
    list(side = "right", pct = year_row$pm25_pct, col = pm25_color)
  )
  ...
  list(
    grey = dplyr::bind_rows(grey_layers),
    fill = dplyr::bind_rows(fill_layers),
    outline = dplyr::bind_rows(outline_layers)
  )
}

bl_plot_lung_exceedance_year <- function(
    year_row,
    outlines,
    centroids,
    yranges,
  ...
  layers <- bl_lung_year_layers(
    year_row, outlines, yranges, no2_color, pm25_color
  )
  ...
  # Day counts sit above the trachea so they never cross the airway ladder.
  days_y <- BL_LUNG_TRACHEA_TOP + 0.035
  year_y <- BL_LUNG_TRACHEA_TOP + 0.166

  label_df <- data.frame(
    year = as.character(year_row$year),
    cx_left = cx_left,
    cx_right = cx_right,
    year_y = year_y,
    no2_days_y = days_y,
    pm25_days_y = days_y,
    no2_name_y = yr_left[1] - 0.07,
    pm25_name_y = yr_right[1] - 0.07,
    no2_fill_top = yr_left[1] + (yr_left[2] - yr_left[1]) * year_row$no2_pct / 100,
    pm25_fill_top = yr_right[1] + (yr_right[2] - yr_right[1]) * year_row$pm25_pct / 100,
    pct_label_gap = 0.085,
    stringsAsFactors = FALSE
  )
  label_df$no2_pct_y <- label_df$no2_fill_top + label_df$pct_label_gap
  label_df$pm25_pct_y <- label_df$pm25_fill_top + label_df$pct_label_gap
  label_df$no2_pct_label <- paste0(year_row$no2_pct, "%")
  label_df$pm25_pct_label <- paste0(year_row$pm25_pct, "%")
  label_df$no2_days_label <- paste0(year_row$no2_days, " days")
  label_df$pm25_days_label <- paste0(year_row$pm25_days, " days")
  label_df$no2_name_label <- "NO[2]"
  label_df$pm25_name_label <- "PM[2.5]"
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::geom_text(
      data = label_df,
      ggplot2::aes(x = 0.5, y = year_y, label = year),
      fontface = "bold", size = sizes$year
    ) +
    ggplot2::geom_text(
      data = label_df,
    ...
    ggplot2::coord_fixed(
      ratio = 1, xlim = c(-0.05, 1.05), ylim = c(-0.02, 1.24), clip = "off"
    ) +
    ggplot2::theme_void(base_size = 12) +
    ggplot2::theme(plot.margin = ggplot2::margin(2, 6, 10, 6))
}
...
bl_plot_lung_exceedance <- function(
  ...
  if (!requireNamespace("cowplot", quietly = TRUE)) {
    stop("Install cowplot for lung exceedance plot.", call. = FALSE)
  }
  ...
  year_plots <- lapply(seq_len(nrow(plot_df)), function(i) {
    bl_plot_lung_exceedance_year(
      plot_df[i, , drop = FALSE],
      outlines, centroids, yranges,
      no2_color, pm25_color, sizes
    )
  })

  n_cols <- as.integer(years_per_row)
  n_rows <- ceiling(length(year_plots) / n_cols)
  row_grids <- lapply(seq_len(n_rows), function(r) {
    idx_start <- (r - 1L) * n_cols + 1L
    idx_end <- min(r * n_cols, length(year_plots))
    row_plots <- year_plots[idx_start:idx_end]
    if (length(row_plots) < n_cols) {
      row_plots <- c(row_plots, rep(list(NULL), n_cols - length(row_plots)))
    }
    cowplot::plot_grid(
      plotlist = row_plots,
      ncol = n_cols,
  ...
  grid <- cowplot::plot_grid(
    plotlist = row_grids,
    ncol = 1,
    rel_heights = rep(1, length(row_grids))
  )
